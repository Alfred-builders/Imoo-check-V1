  ...
  return (
    ...
    >
      <div className="w-8 flex justify-center">
        <div className={`h-7 w-7 rounded-full flex items-center justify-center shrink-0 ${t.type_personne === 'morale' ? 'bg-blue-100' : 'bg-slate-100'}`}>
          {t.type_personne === 'morale'
            ? <Building2 className="h-3.5 w-3.5 text-blue-700" />
            : <span className="text-[10px] font-bold text-slate-700">{(t.prenom?.[0] || t.nom[0]).toUpperCase()}</span>
          }
        </div>
      </div>
      ...
      <div className="w-24">
        <Badge variant="outline" className={`text-[10px] font-normal ${t.type_personne === 'morale' ? 'text-blue-600 border-blue-200' : 'text-slate-600 border-slate-200'}`}>
